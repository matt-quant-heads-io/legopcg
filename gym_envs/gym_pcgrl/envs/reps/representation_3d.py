...
from gym_envs.gym_pcgrl.envs.helper import gen_random_map_3d
import numpy as np
...
class Representation3D:
    ...
    def _is_valid_location(self, tile_y, tile_x, tile_z):
        ...
        if np.any(
            self._map[self.y, self.x : self.x + tile_x, self.z : self.z + tile_z] != 0
        ):
            return False
        ...
        if self.y - 1 >= 0:
            ...
            if np.all(
                self._map[
                    self.y - 1, self.x : self.x + tile_x, self.z : self.z + tile_z
                ]
                == 0
            ):
                return False
